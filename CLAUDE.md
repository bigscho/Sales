...
- **Cross-booking setter credit**: a closer's name in "Booked by" on ANOTHER closer's calendar sets `setterId` to that closer (they were acting as a setter, leadSource stays fed); on their OWN calendar it's `self_sourced` with no setter. Never auto-create a setter TeamMember row for a closer's name.
