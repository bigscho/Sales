...
## Closer presenter reconciliation (Aug 2026) — who RAN the demo vs who's credited
- `closerId` comes from the Calendly host, but closers cover each other's double-bookings, so the *credited* closer often isn't the one who *presented*. The Fireflies sync now writes an **advisory** `Demo.detectedCloserId` + `detectedCloserShare` = the closer holding the majority of transcript talk (≥60% of INTERNAL talk; null when co-run/ambiguous). **Advisory ONLY — never auto-applied to `closerId`** (respects the shadowing rule + "automation must not overwrite manual corrections").
- Reconciliation happens **inline on `/demos`** (no separate page — Colin, Aug 17: "low key, from the same dashboard"). The closer cell:
  - **Admin**: an editable closer dropdown; when `detectedCloserId !== closerId`, an amber **"▲ ran: &lt;name&gt; &lt;share&gt;% →"** badge one-click reassigns to the detected presenter.
  - **Non-admin closer (EOD self-claim)**: read-only name + a green **"✋ Claim — you ran this"** button that appears ONLY when the transcript detected THEM (`detectedCloserId === session.memberId`) and they're not yet credited. A passive "▲ ran: &lt;name&gt;" note shows when someone else was detected. Server-side the PATCH lets a non-admin closer set `closerId` ONLY to their own `memberId` AND only when detection backs it — evidence-gated so no one can pull a teammate's demo/commission.
- **Reassigning a demo's closer cascades to the linked `Deal.closerId`** (audit `closer_reassigned` on both demo + deal) so commission (payroll reads `deal.closerId` on demand) and the close move too — **"moves both"** (Colin, Aug 17). Day-lock blocks reassignment, protecting confirmed/paid weeks (do EOD claims before the Lock Week).
- Detection lives in `api/sync/fireflies/route.ts` (`detectPresenter` / `matchCloserId`): candidates are role=closer only (setters/prospects never match), matched by first/full name; talk-share proxied by sentence count. A backfill pass in the same sync fills detection for already-recorded demos (last 2wk) from the fetched transcripts. Schema: two nullable cols on Demo, applied via `db push`.
- **KNOWN pre-existing bug this surfaced (not caused by this feature):** the transcript→demo matcher fuzzy-matches on name substrings — the "Jan Norris" demo linked to the **"Janice Pruss"** transcript because the title contains "jan" — so a mis-linked demo's detection reflects the wrong call. The matcher (`matchTranscriptToDemo` / the sync's name-in-title check) needs a tighter name match; tracked separately.

## CRITICAL: Immutable Week History (Aug 2026) — do not regress
- **A booking row never leaves its week and `bookedAt` is NEVER bumped after row creation.** A real reschedule/rebook freezes the old row (`supersededAt` set; pending demo → `rescheduled`; showed/no_show/cancelled untouchable) and creates a successor row (`rescheduledFromId` link) in the new week, credited to the most-recent setter. The Calendly-webhook successor takes the NEW invitee's name/email/phone (fallback to the old row's when absent) — never carry the old name forward; Calendly browser-autofill puts the previous prospect's name on back-to-back bookings and the corrected rebook must win (Erica Anderson, 2026-09-03). Same-event GCal drags of still-pending demos move in place without touching `bookedAt`.
- **Reschedule successors INHERIT the original row's `bookedAt`** (like they already inherit `leadSource`) — a reschedule of a still-pending demo is the SAME booking moving, so its *activity* credit ("New Bookings") stays in the week it was first booked. EXCEPTION: a re-engagement after a terminal outcome (showed/no_show/cancelled) is a genuinely new meeting and its successor gets a fresh `bookedAt = now`. The successor's `weekId`/demo still live in the new week, so *results* (shows/pending) land there — activity and results deliberately split. (Fixed 2026-08-17, commit `4ffea0b`; see [[project_sales_scoreboard_reschedule_newbooking_fix_2026_08_17]] — was stamping `now` on every successor, inflating the rebooking week's New Bookings.)
- **"New Bookings" activity counts booking EVENTS via `Booking.isBookingEvent`, NOT `supersededAt: null` (changed 2026-08-27).** The old `supersededAt: null` filter dropped a terminal original the moment its prospect rebooked, so past weeks silently shrank; the other five surfaces (kpis/setter-game/credits/slack setter confirm+eod/demos-today) had no such filter and double-counted rebooks. Now every activity surface routes through **one** helper — `newBookingActivityWhere()` in `src/lib/booking-activity.ts` — which filters `isBookingEvent: true`. Never re-derive this where-clause inline; the whole point is that all surfaces agree. Dedup matchers still (correctly) filter `supersededAt: null` — that is unrelated to counting.
- **`isBookingEvent` semantics:** true = a real booking event (an original booking, OR a genuine re-book after a terminal outcome — new setter effort earns its own credit in the rebooker's week). false = a pending-reschedule successor (same meeting moved to a new time, inherited `bookedAt`) so a time change never double-counts. Set at write time (gcal split = always terminal → true; Calendly-webhook reschedule = true iff `bookedAt` is fresh vs the parent's) and frozen forever. Consequence (intended, Colin 2026-08-27): a no-show that rebooks = **2 bookings but 1 show** — activity is a leading/effort metric (>1 per prospect OK), a show is an outcome counted once by `demoDate` (results counting is unchanged).
- Because `isBookingEvent` and `bookedAt` are frozen at creation and never restated, a past week's New Bookings count is stable forever — this is what fixed the "past weeks shrink" bug. Never mutate a superseded row.
- The "as-booked" number (dashboard + scoreboard) = live rows `createdAt` in the period; the immutable "what we saw live" reference. It still filters `supersededAt: null` and counts by `createdAt` (reschedule day), so for a rescheduled booking it can differ from New Bookings — the UI shows both when they diverge.
...
- **Scoreboard page = two-view toggle** (Aug 13): Setter Scoreboard | Closer Scoreboard segmented control at the top of /scoreboard (deep-linkable with `?view=closers`). Setter view = summary cards + leaderboards (the show-rate-rep card was REMOVED Aug 13 — it displayed Belayneh's bonus comp to the whole team; do not re-add, his pay lives on /payroll). **GCal acceptance is paired INTO the Shows leaderboard (Sep 13): each setter row stacks a Show bar (StatusBar: showed/pending/no-show/cancelled) over a GCal `InviteBar` (accepted/no-response/declined) — a static RSVP breakdown, NOT a demo-timeline signal — each with its own rate, so you read show-rate-vs-acceptance correlation straight down the ranked list. The standalone "GCal Invite Accepts" third card (commit 4a101d7) was RETIRED into this — do not re-add it. `/api/scoreboard` returns per-setter `invites {accepted, declined, captured}` (no-response = captured−accepted−declined); accept rate = accepted/captured (tracked invites only, so coverage depends on the closer's calendar being shared with the tracking service account). Boards are side-by-side (New Bookings | Shows & GCal), do not stack them full-width — Colin wants no vertical scroll.** Closer view = closer summary cards (derived client-side from the same closerBoard rows as the table, so they can't disagree) + the closer table with drill-downs. The demos tile is labeled "Demos Booked" (Colin, Aug 13: "Demos Run" was misleading — the closers didn't run no-shows; it counts demos booked onto their calendars). Both views share the daily/weekly/monthly/all-time dimension toggle.
...
- **Cross-channel duplicate defense (Sep 14, 2026 — do not regress)**: the same meeting arrives via BOTH the Calendly webhook and the 10-min GCal sync with no shared key, so identity matching is the only dedup. Rules now: (1) the gcal twin/rebook matchers compare against ALL external attendee emails, never just `attendees[0]` (a guest's email can be listed first — Douglas Harper 09-14 dup); (2) same name + same normalized phone overrides the different-emails-=-different-people veto in BOTH matchers (typo'd/alias/guest emails — both channels capture the Calendly form phone); (3) phone alone NEVER matches — office-mates share numbers (Rayburn/Luker); (4) `flagDuplicatePairs()` tripwire in the gcal sync alerts Slack once per pair (AuditLog `duplicate_pair_flagged`) whenever two live rows for the same person sit within an hour; (5) "Canceled:"-renamed mirror events (Calendly renames instead of deleting, status stays confirmed) cancel a pending row and never mint one. 9 historical dup pairs repaired 09-14 (audit `duplicate_booking_repair`): 6 deleted w/ DismissedEvent blocks, 3 cancel-then-rebook pairs converted to reschedule chains. ~23 stale pending same-name pairs remain in the March 2026 `source='auto'` import era — left alone deliberately.
- **Demo status write rules (Sep 14, 2026 — do not regress)**: `/api/demos` PATCH status = admin/closer only; `showed`/`no_show` are REJECTED before the meeting time (15-min grace; cancelled/rescheduled may be recorded early). POST `bulk_confirm` = admin-only, status whitelisted, skips not-yet-run demos + locked days, audit-logs per-demo `status_update` rows, and stamps `confirmedBy` with the session name. The `/demos` All Showed / bulk-bar buttons render for admins only. Cause: the endpoint previously had NO auth/lock/date checks and NO audit — on 2026-09-14 unaudited bulk_confirm sweeps marked all 30 of Tuesday's pending demos (plus Monday's not-yet-run ones) as `showed`, putting phantom shows on the scoreboard.
- **Day locking** system
...
Show-rate rep work surface at `/confirmations` (role: `show_rate_rep` + admin). Three touchpoints sent into setter-created iMessage groups via SendBlue (AI Agent line +13137683591 — reply-only, can't cold-initiate; the setter adding the line to the group at booking is what registers it):
- **Origination** (webhook-driven, Sep 2026): instant reply to the setter's first text in a brand-new group — "Thanks {setter}, talk {today|tomorrow|weekday} {prospect}. Definitely check out the FAQ^!" (text-only; the ^ points at the setter's message, which carries the FAQ link). Fires from `/api/webhooks/sendblue` → `maybeSendOriginationReply` (`src/lib/confirmations/origination.ts`) — NEVER from polling (a poll-discovered group is stale by definition; freshness cap 60 min on `SendblueGroup.createdAt` guarantees the reply is near-instant or absent). Matching is TWO-TIER: phone match (same as t1/day-of) → full personalized copy; else TIME match — exactly ONE unclaimed, ungreeted, upcoming booking created within ±30 min of the group → GENERIC copy with no names ("Thanks, talk tomorrow. Definitely check out the FAQ^!"; a wrong name would sit visibly contradicted in the thread, the day is protected by the exactly-one rule); zero or 2+ candidates → silence. The time match feeds the greeting ONLY — it never writes `SendblueGroup.bookingId`, so T-1/day-of still require the phone match. Match tier logged in `ConfirmationSend.variant` (phone_matched | time_matched). Other gates: once-per-prospect-ever + per-booking retrigger guard via ConfirmationSend, demo pending + upcoming, prospect-local 7am–10pm; no match at all = silence (ops/non-prospect groups). REQUIRES the webhook URL (`https://barn.grsfd.ai/api/webhooks/sendblue`) registered in the SendBlue dashboard, and real sends need `CONFIRMATIONS_ORIGINATION_LIVE=true` ON TOP of `SENDBLUE_LIVE` (dry-run rows otherwise — prod is already SENDBLUE_LIVE for t1/day-of, so the per-touchpoint flag is the actual switch).
...
- **Group discovery**: poll-based (`syncGroupsFromApi` — `GET /api/v2/messages` before every worklist build) for t1/day-of, PLUS the inbound webhook (`src/app/api/webhooks/sendblue/route.ts`) — no longer optional: the origination touchpoint only fires from it. Day-of sends as TWO messages: video first, text second (text owns the inbox preview).
...
- CONFIRMATIONS_ORIGINATION_LIVE ("true" = origination webhook reply sends real texts; unset = dry-run rows even when SENDBLUE_LIVE is true) — NOT YET SET
