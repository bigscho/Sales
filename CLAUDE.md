...
- All dedup matchers **and the scoreboard "New Bookings" + "as-booked" counts** filter `supersededAt: null` — only the live row of a reschedule chain ever counts, so a demo rescheduled N times is not counted N times. Never mutate a superseded row — it is frozen history and the reason past weeks' numbers no longer restate.
- **SEQUENCING TRAP (do not regress):** the `supersededAt: null` filter excludes the frozen original, so it is only safe once live successors' `bookedAt` is anchored to the original week. Any future backfill/migration must re-anchor `bookedAt` BEFORE (or with) shipping such a filter — filter-first would drop cross-week reschedules out of their original week, re-creating the old "setters lose reschedules" bug.
- The "as-booked" number (dashboard + scoreboard) = live rows `createdAt` in the period; the immutable "what we saw live" reference. Note it counts by `createdAt` (reschedule day, not backfilled), so for a rescheduled booking it can differ from New Bookings (which counts by the inherited `bookedAt`) — the UI shows both when they diverge.
