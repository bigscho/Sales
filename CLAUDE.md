# Grassfed Sales Dashboard — Session Handoff (May 24, 2026)
...
6. **Calendly webhook event type filter silently dropping all new bookings** — FIXED (May 24, commit `4bc99c1`): the event type filter in `/api/webhooks/calendly/route.ts` was hardcoded to old event type names (`"Grassfed Demo"`, `"Grassfed E-Mailers Setup"`) that no longer exist. All current event types (`"Grassfed | Farm, Just-Listed, Just Closed"`, etc.) matched none of the keywords, so every `invitee.created` since the rename was silently dropped with a 200 OK — Calendly never retried, all bookings fell through to gcal_sync with no real-time Slack. Fix: updated filter keywords to `farm | just | demo | e-mailers | setup`. Also added audit log entry (`calendly_non_demo_skipped`) on skips so this never goes silent again. **Watch out**: if event type names change in Calendly, check this filter. The audit log is the canary — if you see `calendly_non_demo_skipped` entries piling up, the filter needs updating.
