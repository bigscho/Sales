...
- **Closer view scoping (non-admin closers)**: middleware limits them to /demos + /deals (+ /api/closer stats); "/" redirects to /demos. /api/demos, /api/demos/today, /api/deals return ONLY their own rows and no unlinked-payments feed. Demo DELETE + day lock/unlock are admin-only server-side. Demos/deals pages render read-mostly for closers (status + FED/SELF toggle only). Audit `performedBy` = session name — do not regress to hardcoded "admin".
