...
- **Closer view scoping**: non-admin closers see only their own demos/deals (API-level filter), no company dashboard/scoreboard/KPIs/unlinked-payments feed, and read-mostly pages (status + FED/SELF edits; no add/delete/bulk/locks/reassign). Demo DELETE and day locks are admin-only server-side. All demo/deal edits audit-log the session user's name.
