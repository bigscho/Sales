...
export function SyncButton() {
  ...
  return (
    <div className="flex items-center gap-3">
      {result && (
        <span className="text-xs text-[var(--muted-foreground)]">{result}</span>
      )}
      <button
        onClick={handleSync}
        disabled={syncing}
        className="h-9 px-4 rounded-lg bg-[var(--primary)] text-white text-sm font-medium hover:bg-green-700 transition-colors disabled:opacity-50"
      >
